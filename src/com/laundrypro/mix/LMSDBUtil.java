...
public class LMSDBUtil {
	...
	public static boolean cutomerDetails(String Name, String ContactNo, String Address) {
		...
		try {
			...
			String sqlq1 = "SELECT COUNT(*) FROM Customer WHERE ContactNo = '" + ContactNo + "'";
			...
			if (count > 0) {
				...
				String sqlq2 = "INSERT INTO Customer values('" + ContactNo + "','" + Name + "','" + Address + "')";
				int rs = stmt.executeUpdate(sqlq2);
	...
	public static List<Customer> ValidateCustomer(String contactNo) {
		...
		try {
			...
			String sql = "SELECT * FROM customer WHERE contactNo = '" + contactNo + "'";
			rs = stmt.executeQuery(sql);
	...
	public static boolean UserDetails(String Name, String Username, String Address, String NIC, String Password,
			String Type) {
		boolean isSucess = false;
		...
		try {
			...
			String sql = "INSERT INTO LMS_User values('" + Name + "','" + Username + "','" + Address + "','" + NIC
					+ "','" + Password + "','" + Type + "')";
	...
	public static boolean UpdateOrder(String refNo, String finalAmount, String paidAmount, String billDate,
			String dueDate, String state) {

		try {

			con = DBconnect.getConnection();
			stmt = con.createStatement();
			String sql = "UPDATE bill SET finalAmount='" + finalAmount + "',paidAmount='" + paidAmount + "',billDate='"
					+ billDate + "',dueDate='" + dueDate + "',state='" + state + "' where refNo='" + refNo + "'";
			int rs = stmt.executeUpdate(sql);
	...
	/* TRACK ORDER */
		public static List<TrackOrder> TrackOrder(String refno) {
			...
			try {
				...
				con = DBconnect.getConnection();
				stmt = con.createStatement();
				String sql = "SELECT b.refNo, b.finalAmount, b.paidAmount, b.billDate, b.dueDate, b.state, cr.contactNo, c.name, c.address FROM bill b, customer_ref cr, customer c WHERE b.refNo = cr.refNo AND cr.contactNo = c.contactNo AND b.refNo='" + refno + "'";
				rs = stmt.executeQuery(sql);
				...
				while (rs.next()) {
					...
					TrackOrder to = new TrackOrder(refNo, finalAmount, paidAmount, billDate, dueDate, state, contactNo, name, address);
					trackorder.add(to);
				}
		...
		public static List<OrderItem> OrderItem(String refNo) {
			...
			try {
				...
				while (rs.next()) {
					...
					OrderItem oi = new OrderItem(description, serviceType, unitPrice, quantity);
					orderitem.add(oi);
				}
		...
		public static List<Customer> CustomerDetails(String refNo) {
			...
			try {
				...
				con = DBconnect.getConnection();
				stmt = con.createStatement();
				String sql = "SELECT * FROM customer WHERE contactNo = (SELECT contactNo FROM customer_ref WHERE refNo='"+refNo+"')";
				rs = stmt.executeQuery(sql);
		...
		public static boolean CloseBill(String refNo, String paidAmount) {
			...
			try {
				...
				con = DBconnect.getConnection();
				stmt = con.createStatement();
				String sql = "UPDATE bill SET paidAmount='"+paidAmount+"' where refNo='"+refNo+"'";
				int rs = stmt.executeUpdate(sql);
